...
class ImageTrainer(Trainer):
  ...
  def save_samples(self, train_iter):
    ...
    for i in range(self.n_samples):
      ...
      axes[1,i].imshow(x[i]/256, cmap='gray')
      axes[2,i].imshow(x_reconstr[i], cmap='gray')
      #print(samples[i])
      #print(x[i]/256)
      #print(x_reconstr[i])
...
def make_prior(args):
  flow = nux.NonlinearCoupling(n_layers=5,
                               ...
                               kind="spline",
                               with_affine_coupling=True)
  return nux.Sequential([nux.Flatten(), flow], prior=nux.DiscreteUnitLogisticPrior())

################################################################################################################

def create_flow(args):
  args.save_path = f"ds_last_1_5latents_nocirc"
