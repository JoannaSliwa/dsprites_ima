...
class Trainer():
  ...
  def __init__(self,
    ...
    self.additional_metrics_train = None # dict(mcc_spearman, mcc_pearson, cima)
    self.additional_metrics_test = None  # dict(mcc_spearman, mcc_pearson, cima)
  ...
  def mcc_calc(self, inputs):
    x = inputs["x"]
    latents = inputs["latents"]
    print('mcc',x.shape)
    Ncomp = latents.shape[1]
    print(Ncomp)
        

    source=latents
    recov =self.jitted_to_latent(x,self.params, self.eval_key)
    #recov=jax.scipy.stats.logistic.cdf(recov)
    #source=jax.scipy.stats.logistic.cdf(source)
    print(source)
    print(recov[:,:Ncomp])
    rho, _ = np.abs(spearmanr(recov, source))
    CorMat = rho[:Ncomp, Ncomp:]
    ii = linear_sum_assignment(-1 * CorMat)
    mcc_spearman_value = CorMat[ii].mean()
    
    print('here')

    CorMat = (np.abs(np.corrcoef(recov.T, source.T)))[:Ncomp, Ncomp:]
    ii = linear_sum_assignment(-1 * CorMat)
    mcc_pearson_value = CorMat[ii].mean()
    print(mcc_spearman_value, mcc_pearson_value)
    print(mcc_spearman_value.item(), mcc_pearson_value.item())
    return mcc_spearman_value.item(), mcc_pearson_value.item()
  ...
  def train(self, train_iter, train_key=None):
    ...
    inputs = dict(x=x, latents=latents, rng_key=keys, i=jnp.arange(self.n_train_steps, self.n_train_steps + n_grad_steps))
    if self.data_augmentation:
    ...
    if self.train_for_loop:
      ...
      for i in pbar:
        print(i, "in pbar for loop")
        print('inputs',inputs["x"].shape)
        _inputs = jax.tree_util.tree_map(lambda x: x[i], inputs)
        print('_inputs',_inputs["x"].shape)
        self.train_state, out = self.grad_step(self.train_state, _inputs, catch_nan=True)
        train_loss = out[0].mean()
        cima_value = out[1]["Ls"]
        print(cima_value)
        print(cima_value.item())
        outs.append(out)
        pbar.set_description(f"loss: {train_loss}")
        if jnp.isnan(train_loss):
          break
        mcc_spearman_value, mcc_pearson_value = self.mcc_calc(_inputs)
        mcc_s_outs.append(mcc_spearman_value)
        mcc_p_outs.append(mcc_pearson_value)
        cimas_outs.append(cima_value.item())

      train_losses, aux, grad_summary = jax.tree_util.tree_map(lambda *xs: jnp.array(xs), *outs)
      train_mcc_s = jax.tree_util.tree_map(lambda xs: jnp.array(xs), mcc_s_outs)
      train_mcc_p = jax.tree_util.tree_map(lambda xs: jnp.array(xs), mcc_p_outs)
      train_cima = jax.tree_util.tree_map(lambda xs: jnp.array(xs), cimas_outs)
    else:
    ...
    if self.using_map:
      print('1',self.train_metrics, util.tree_shapes(self.train_metrics))
      self.train_metrics = util.tree_concat(self.train_metrics, train_metrics, axis=1)
    else:
      print('2',self.train_metrics, util.tree_shapes(self.train_metrics))
    ...
    additional_metrics_train = dict(mcc_spearman=jnp.array(mcc_s_outs), mcc_pearson=jnp.array(mcc_p_outs), cima=jnp.array(cimas_outs))
    if self.using_map:
      print('3',self.additional_metrics_train, util.tree_shapes(self.additional_metrics_train))
      self.additional_metrics_train = util.tree_concat(self.additional_metrics_train, additional_metrics_train, axis=1)
    else:
      print('4',self.additional_metrics_train, util.tree_shapes(self.additional_metrics_train))
      self.additional_metrics_train = util.tree_concat(self.additional_metrics_train, additional_metrics_train, axis=0)
  ...
  def test(self, get_test_iter):
    ...
    try:
      ...
      while i < 100:
        ...
        if self.using_map:
          ...
          keys = rng_key
        inputs = dict(x=x, latents=latents, rng_key=keys)
        ...
        mcc_spearman_value, mcc_pearson_value = self.mcc_calc(inputs)
        cima_value = aux["Ls"].item()
        ...
        additional_metrics = dict(mcc_spearman=jnp.array(mcc_spearman_value), mcc_pearson=jnp.array(mcc_pearson_value), cima=jnp.array(cima_value))
        if self.using_map:
          print('ahh here')
          additional_metrics = jax.tree_util.tree_map(lambda x: x[:, None], additional_metrics)
          additional_metrics_test = util.tree_concat(additional_metrics_test, additional_metrics, axis=1)
        else:
          print('additional')
          print(additional_metrics_test)
          print(additional_metrics)
          additional_metrics_test = util.tree_hstack(additional_metrics_test, additional_metrics)
          print(additional_metrics_test)
    ...
    if self.using_map:
      additional_metrics_test = jax.tree_util.tree_map(lambda x: jnp.mean(x, axis=1), additional_metrics_test)
      additional_metrics_test = jax.tree_util.tree_map(lambda x: x[:,None], additional_metrics_test)
      self.additional_metrics_test = util.tree_concat(self.additional_metrics_test, additional_metrics_test, axis=1)
    else:
      additional_metrics_test = jax.tree_util.tree_map(jnp.mean, additional_metrics_test)
      self.additional_metrics_test = util.tree_hstack(self.additional_metrics_test, additional_metrics_test)
  ...
  @property
  def additional_metrics_train_path(self):
    return os.path.join(self.losses_path, "train_metrics.csv")

  @property
  def additional_metrics_test_path(self):
    return os.path.join(self.losses_path, "test_metrics.csv")
  ...
  def checkpoint(self):
    ...
    additional_metrics_train = pd.DataFrame(self.additional_metrics_train)
    additional_metrics_test = pd.DataFrame(self.additional_metrics_test, index=self.test_eval_times)

    # Save the losses
    train_losses.to_csv(self.train_losses_path, index=False, header=False)
    test_losses.to_csv(self.test_losses_path, header=False)

    additional_metrics_train.to_csv(self.additional_metrics_train_path, index=False, header=False)
    additional_metrics_test.to_csv(self.additional_metrics_test_path, header=False)
  ...
  def to_latent(self, x, params, rng_key):
    z, _ = self.flow(x, params=params, rng_key=rng_key, no_lcc=True, is_training=False)
    return z
 

  @property
  def jitted_to_latent(self):
    if hasattr(self, "_jitted_to_latent") == False:
      self._jitted_to_latent = jax.jit(self.to_latent)
    return self._jitted_to_latent
